...
const CargoDeleteModal: React.FC<CargoDeleteModalProps> = (
    {
        cargo,
        removeCargo,
        isVisible,
    ...
    const [deleteCargo, isDeleteLoading, error] = useRequest(async (cargo: Cargo) => {
        setIsToastVisible(false);
        await CargoApiClient.deleteCargo(cargo);
                            ...
                            <div className="sm:flex sm:items-start">
                                ...
                                <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                    <h3 className="text-base font-semibold leading-6 text-gray-900"
                                        id="modal-title">Удаление груза</h3>
                                    <div className="mt-2">
                                        <p className="text-sm text-gray-500">
                                            Вы уверены, что хотите удалить груз "{cargo.name}"
                                            ({cargo.departureCity} - {cargo.arrivalCity})?
                                            Вся информация о грузе будет удалена навсегда. Это действие не может быть
                                            отменено.
                                        </p>
                                    </div>
                                </div>
...
export default CargoDeleteModal;